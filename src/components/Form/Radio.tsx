...
const Radio: React.FC<Props> = ({ children, className, id, hideRadio, ...props }) => {
  return (
    <div className={`relative flex text-sm overflow-hidden ${className}`}>
      ...
      <label className={`w-full opacity-50 peer-checked:opacity-100 peer-checked:text-slate-900 dark:peer-checked:text-white cursor-pointer transition-colors ${hideRadio ? '' : 'pl-2'}`}
        htmlFor={id}
      >
        {children}
      </label>
